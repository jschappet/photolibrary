...
// @LINE:42
def update(photo:Long): Call = {
   Call("PUT", _prefix + { _defaultPrefix } + "photos/" + implicitly[PathBindable[Long]].unbind("photo", photo))
}
                                                

// @LINE:41
def add(): Call = {
   Call("POST", _prefix + { _defaultPrefix } + "photos/")
}
...
def index(): Call = {
   () match {
// @LINE:39
case () if true => Call("GET", _prefix + { _defaultPrefix } + "photos")
...
// @LINE:42
def update : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.Photos.update",
   """
      function(photo) {
      return _wA({method:"PUT", url:"""" + _prefix + { _defaultPrefix } + """" + "photos/" + (""" + implicitly[PathBindable[Long]].javascriptUnbind + """)("photo", photo)})
      }
   """
)
...
// @LINE:41
def add : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.Photos.add",
   """
      function() {
      return _wA({method:"POST", url:"""" + _prefix + { _defaultPrefix } + """" + "photos/"})
      }
   """
)
...
// @LINE:39
def index : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.Photos.index",
   """
      function() {
      if (true) {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "photos"})
      }
      if (true) {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "photos/"})
      }
...
// @LINE:42
def update(photo:Long): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.Photos.update(photo), HandlerDef(this, "controllers.Photos", "update", Seq(classOf[Long]), "PUT", """""", _prefix + """photos/$photo<[^/]+>""")
)
                      

// @LINE:41
def add(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.Photos.add(), HandlerDef(this, "controllers.Photos", "add", Seq(), "POST", """""", _prefix + """photos/""")
)
                      

// @LINE:39
def index(): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   controllers.Photos.index(), HandlerDef(this, "controllers.Photos", "index", Seq(), "GET", """ Photo""", _prefix + """photos""")
)
